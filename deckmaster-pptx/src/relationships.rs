...
impl Relationships {
    ...
    pub fn load_slide_relationships(
        ...
        let rels_path =
            slide_relationships_path(slide_target);

        let xml =
            package.read_string(&rels_path)?;
...
fn slide_relationships_path(slide_target: &str) -> String {
    let normalized =
        slide_target.trim_start_matches('/');

    let slide_path =
        normalized.strip_prefix("ppt/").unwrap_or(normalized);

    let Some((dir, file_name)) =
        slide_path.rsplit_once('/')
    else {
